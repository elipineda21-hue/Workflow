...
const ACCENT     = rgb(30/255, 107/255, 184/255);   // #1E6BB8
const WHITE      = rgb(1, 1, 1);
const LIGHT_GRAY = rgb(240/255, 244/255, 248/255);  // #F0F4F8
const MID_GRAY   = rgb(107/255, 114/255, 128/255);  // #6B7280
const DARK_TEXT  = rgb(13/255, 34/255, 64/255);
const RULE_COLOR = rgb(226/255, 232/255, 240/255);  // #E2E8F0
...
export async function buildSubmittalPDF({
  ...
  onProgress("Building cover page...");
  {
    ...
    // Accent top bar
    pg.drawRectangle({ x: 0, y: PAGE_H - 42, width: PAGE_W, height: 42, color: ACCENT });
    pg.drawText(company.toUpperCase(), {
      x: MARGIN, y: PAGE_H - 30, size: 11, font: fontBold, color: WHITE,
    });

    // Navy hero block
    const heroTop = PAGE_H - 42;
    const heroH = 260;
    pg.drawRectangle({ x: 0, y: heroTop - heroH, width: PAGE_W, height: heroH, color: NAVY });

    // Title
    pg.drawText("SUBMITTAL PACKAGE", {
      x: MARGIN, y: heroTop - 80, size: 28, font: fontBold, color: WHITE,
    });
    // Project name
    pg.drawText(projectName, {
      x: MARGIN, y: heroTop - 115, size: 14, font: fontRegular, color: WHITE,
    });

    // Thin rule
    pg.drawRectangle({ x: MARGIN, y: heroTop - 135, width: CONTENT_W * 0.8, height: 1, color: ACCENT });

    // Meta lines
    pg.drawText("Low Voltage Security Systems", {
      x: MARGIN, y: heroTop - 155, size: 10, font: fontRegular, color: rgb(0.8, 0.84, 0.88),
    });
    const sectionTags = sections.map(s => s.title.replace(/ SYSTEM$/, "")).join("  ·  ");
    pg.drawText(sectionTags, {
      x: MARGIN, y: heroTop - 172, size: 9, font: fontRegular, color: rgb(0.8, 0.84, 0.88),
    });

    pg.drawText(`Date: ${date}`, {
      x: MARGIN, y: heroTop - 200, size: 10, font: fontRegular, color: rgb(0.8, 0.84, 0.88),
    });
    pg.drawText(`Prepared by: ${company}`, {
      x: MARGIN, y: heroTop - 216, size: 10, font: fontRegular, color: rgb(0.8, 0.84, 0.88),
    });
    ...
    rows.forEach(([label, value], i) => {
      ...
      pg.drawRectangle({ x: MARGIN, y: rowY - 6, width: CONTENT_W, height: rowH, color: LIGHT_GRAY });
      pg.drawRectangle({ x: MARGIN, y: rowY - 6, width: CONTENT_W, height: 0.3, color: RULE_COLOR });
      pg.drawText(label, { x: MARGIN + 8, y: rowY + 3, size: 8.5, font: fontBold, color: ACCENT });
    ...
    pg.drawText(
      ...
      { x: MARGIN, y: MARGIN + 20, size: 8, font: fontRegular, color: MID_GRAY }
    );
  }
  ...
  onProgress("Building table of contents...");
  {
    ...
    // Header
    pg.drawText("TABLE OF CONTENTS", { x: MARGIN, y: y - 4, size: 16, font: fontBold, color: NAVY });
    const subText = `${projectName} Submittal`;
    const subW = fontRegular.widthOfTextAtSize(subText, 8.5);
    pg.drawText(subText, { x: PAGE_W - MARGIN - subW, y: y, size: 8.5, font: fontRegular, color: MID_GRAY });
    pg.drawRectangle({ x: MARGIN, y: y - 12, width: CONTENT_W, height: 1, color: NAVY });
    y -= 32;
    ...
    for (let si = 0; si < sectionPageData.length; si++) {
      ...
      // Section header bar
      pg.drawRectangle({ x: MARGIN, y: y - 4, width: CONTENT_W, height: 18, color: NAVY });
      pg.drawText(`SECTION ${sn}  ·  ${sec.title}`, {
        x: MARGIN + 8, y: y, size: 9.5, font: fontBold, color: WHITE,
      });
      y -= 24;
      ...
      for (let ii = 0; ii < sec.items.length; ii++) {
        ...
        if (ii % 2 === 1) {
          pg.drawRectangle({ x: MARGIN, y: y - 4, width: CONTENT_W, height: 16, color: LIGHT_GRAY });
        }
        pg.drawRectangle({ x: MARGIN, y: y - 4, width: CONTENT_W, height: 0.3, color: RULE_COLOR });

        pg.drawText(itemNum, { x: MARGIN + 8, y: y, size: 8.5, font: fontBold, color: NAVY });
        pg.drawText(item.name, { x: MARGIN + 40, y: y, size: 8.5, font: fontRegular, color: NAVY });

        if (item.pageCount > 0) {
          const refW = fontRegular.widthOfTextAtSize(pgRef, 8.5);
          pg.drawText(pgRef, { x: PAGE_W - MARGIN - refW, y: y, size: 8.5, font: fontRegular, color: ACCENT });
        } else {
          const skip = "(no spec sheet)";
          const skipW = fontRegular.widthOfTextAtSize(skip, 8);
          pg.drawText(skip, { x: PAGE_W - MARGIN - skipW, y: y, size: 8, font: fontRegular, color: MID_GRAY });
        }
        y -= 18;
        ...
        if (y < MARGIN + 30 && (ii < sec.items.length - 1 || si < sectionPageData.length - 1)) {
          ...
          pg2.drawText("TABLE OF CONTENTS (continued)", { x: MARGIN, y: PAGE_H - MARGIN, size: 14, font: fontBold, color: NAVY });
  ...
  for (let si = 0; si < sectionPageData.length; si++) {
    ...
    // Navy band
    divider.drawRectangle({ x: 0, y: midY - 50, width: PAGE_W, height: 120, color: NAVY });
    // Accent rule below
    divider.drawRectangle({ x: 0, y: midY - 50, width: PAGE_W, height: 6, color: ACCENT });

    divider.drawText(`SECTION ${sn}`, {
      x: MARGIN, y: midY + 30, size: 24, font: fontBold, color: WHITE,
    });
    divider.drawText(sec.title, {
      x: MARGIN, y: midY - 5, size: 16, font: fontBold, color: rgb(0.7, 0.85, 1),
    });
    ...
    for (let ii = 0; ii < sec.items.length; ii++) {
      ...
      if (item.quantity > 1) {
        ...
        divider.drawText(qtyText, {
          x: PAGE_W - MARGIN - qtyW, y: listY, size: 8, font: fontRegular, color: MID_GRAY,
        });
      }
      listY -= 16;
    }
